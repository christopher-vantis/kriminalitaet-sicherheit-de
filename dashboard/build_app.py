...
def main():
    ...
    # Sprachumschaltung: Die Tabelle bildet deutsche Oberflächentexte auf die
    # englische Fassung ab. Sie wird als JSON eingebettet, damit der Wechsel im
    # Browser ohne Neuladen funktioniert.
    sprachdatei = pathlib.Path(__file__).resolve().parent / "sprachtexte.json"
    sprachtexte = json.loads(sprachdatei.read_text(encoding="utf-8")) if sprachdatei.exists() else {"de": {}, "en": {}}
...
I18N_PFAD = pathlib.Path(__file__).resolve().parent / "i18n.json"
...
.kopf{color:#fff;padding:20px 0 18px;position:relative;
...
// ---------------------------------------------------------------- Sprachen
// Die Seite wird auf Deutsch gebaut. Beim Umschalten werden die Textknoten
...
function texteErsetzen(lang){
  const tabelle = SPRACH_TEXTE[lang] || {};
  const geh = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
  const knoten = [];
  while (geh.nextNode()) knoten.push(geh.currentNode);
  knoten.forEach(k => {
    const roh = k.nodeValue;
    if (!roh || !roh.trim()) return;
    const sauber = roh.trim();
    // Exakter Treffer
    if (tabelle[sauber] !== undefined) {
      k.nodeValue = roh.replace(sauber, tabelle[sauber]);
      return;
    }
    // Teiltreffer innerhalb eines Satzes (längste zuerst, damit
    // „mit Migrationshintergrund" nicht von „Migrationshintergrund" gekappt wird)
    let neu = roh;
    Object.keys(tabelle).sort((a,b) => b.length - a.length).forEach(dt => {
      if (dt.length > 3 && neu.indexOf(dt) >= 0) neu = neu.split(dt).join(tabelle[dt]);
    });
    if (neu !== roh) k.nodeValue = neu;
  });
  document.body.dataset.ersetzt = (document.body.dataset.ersetzt || 0) + '|' + lang + ':' + knoten.length;
  // Attribute (Titel, Platzhalter)
  document.querySelectorAll('[title],[placeholder],[aria-label]').forEach(el => {
    ['title','placeholder','aria-label'].forEach(a => {
      ...
      if (w && tabelle[w.trim()] !== undefined) el.setAttribute(a, tabelle[w.trim()]);
    });
  });
}

function spracheSetzen(lang){
  sprache = lang;
  document.documentElement.lang = lang;
  document.querySelectorAll('.sprachwahl button').forEach(b => {
    const aktiv = b.dataset.sprache === lang;
    b.classList.toggle('aktiv', aktiv);
    b.setAttribute('aria-pressed', aktiv ? 'true' : 'false');
  });
  texteErsetzen(lang);
  // Diagramme neu aufbauen, damit Achsen, Legenden und Hinweistexte mitgehen
  if (typeof ansichtNeuAufbauen === 'function') ansichtNeuAufbauen();
}

document.querySelectorAll('.sprachwahl button').forEach(b => {
  b.addEventListener('click', () => {
    spracheSetzen(b.dataset.sprache);
    // Sprache in die Adresse schreiben, damit sich Fassungen verlinken lassen
    const ziel = new URL(window.location);
    if (b.dataset.sprache === 'de') ziel.searchParams.delete('lang');
    else ziel.searchParams.set('lang', b.dataset.sprache);
...
function ansichtNeuAufbauen(){
  const a = (typeof AKTIV !== 'undefined' && AKTIV) ? AKTIV : 'karte';
  if (typeof zeige === 'function') zeige(a, typeof AKTIV_PARAM !== 'undefined' ? AKTIV_PARAM : null);
}
